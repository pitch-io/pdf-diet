...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("output", nargs="?",
                    help="output path (default: <input>.optimized.pdf)")
    ap.add_argument("-p", "--profile", choices=sorted(PROFILES), default="web",
    ...
    ap.add_argument("-d", "--dpi", type=float, default=None,
    ...
    ap.add_argument("-v", "--verbose", action="store_true",
                    help="report what happens to each image")
    ap.add_argument("--version", action="version",
                    version=f"pdfoptimize {__version__}")
    return ap
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not os.path.isfile(args.input):
        print(f"pdfoptimize: no such file: {args.input}", file=sys.stderr)
        return 2

    profile = PROFILES[args.profile]()
    if args.quality is not None:
        if not 0.0 <= args.quality <= 1.0:
            print("pdfoptimize: --quality must be between 0 and 1",
                  file=sys.stderr)
            return 2
        profile.compression_quality = args.quality
    if args.dpi is not None:
        if args.dpi <= 0:
            print("pdfoptimize: --dpi must be positive", file=sys.stderr)
            return 2
    ...
    try:
        ...
        print(f"pdfoptimize: failed to optimize {args.input}: {exc}",
              file=sys.stderr)
        return 1
