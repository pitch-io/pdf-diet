...
class TestRoundTrip:
    def test_output_is_a_valid_readable_pdf(self, gradient_pdf, tmp_path):
        out = tmp_path / "out.pdf"
        optimize_document(gradient_pdf, out, Web())
        with pikepdf.open(out) as pdf:
            assert len(pdf.pages) == 1

    def test_page_count_is_preserved(self, mixed_pdf, tmp_path):
    ...
    def test_it_actually_shrinks(self, gradient_pdf, tmp_path):
        out = tmp_path / "out.pdf"
    ...
    def test_result_reports_per_image_detail(self, gradient_pdf, tmp_path):
    ...
    def test_repeated_runs_are_stable(self, gradient_pdf, tmp_path):
...
class TestDownsampling:
    def test_high_dpi_image_loses_pixels(self, highdpi_pdf, tmp_path):
        out = tmp_path / "out.pdf"
        optimize_document(highdpi_pdf, out, Web())
        after = _images(out)[0]
        assert after["width"] < 800

    def test_low_dpi_image_keeps_its_pixels(self, gradient_pdf, tmp_path):
        out = tmp_path / "out.pdf"
        optimize_document(gradient_pdf, out, Web())
        after = _images(out)[0]
        assert (after["width"], after["height"]) == (800, 450)

    def test_minimal_profile_is_more_aggressive_than_web(self, highdpi_pdf, tmp_path):
...
class TestCropping:
    def test_clipped_image_is_cropped(self, clipped_pdf, tmp_path):
        out = tmp_path / "out.pdf"
        optimize_document(clipped_pdf, out, Web())
        after = _images(out)[0]
        assert after["width"] < 800, "invisible right half should be gone"

    def test_crop_can_be_disabled(self, clipped_pdf, tmp_path):
        profile = Web()
    ...
    def test_cropping_adds_a_corrective_matrix(self, clipped_pdf, tmp_path):
...
class TestSoftMasks:
    def test_opaque_mask_is_dropped(self, opaque_smask_pdf, tmp_path):
        out = tmp_path / "out.pdf"
    ...
    def test_graded_mask_is_preserved(self, smask_pdf, tmp_path):
        out = tmp_path / "out.pdf"
...
class TestProfiles:
    ...
    def test_metadata_is_stripped(self, gradient_pdf, tmp_path):
        out = tmp_path / "out.pdf"
        optimize_document(gradient_pdf, out, Web())
...
class TestCli:
    def test_default_output_path(self, gradient_pdf, capsys):
        assert main([gradient_pdf]) == 0
        expected = os.path.splitext(gradient_pdf)[0] + ".optimized.pdf"
        assert os.path.exists(expected)
        assert "smaller" in capsys.readouterr().out

    def test_explicit_profile_and_quality(self, gradient_pdf, tmp_path):
        out = str(tmp_path / "o.pdf")
        assert main([gradient_pdf, out, "-p", "minimal", "-q", "0.6"]) == 0
        assert os.path.exists(out)

    def test_missing_input_is_reported(self, capsys):
        assert main(["/nonexistent/nope.pdf"]) == 2
        assert "no such file" in capsys.readouterr().err

    def test_quality_out_of_range_is_rejected(self, gradient_pdf, capsys):
        assert main([gradient_pdf, "-q", "5"]) == 2
        assert "between 0 and 1" in capsys.readouterr().err

    def test_negative_dpi_is_rejected(self, gradient_pdf, capsys):
        assert main([gradient_pdf, "-d", "-10"]) == 2
