...
def noisy_image(w: int = 400, h: int = 300, seed: int = 7) -> Image.Image:
    ...
    for y in range(h):
        for x in range(w):
            ...
            px[x, y] = (max(0, min(255, base + rnd.randint(-40, 40))),
...
def build_pdf(path, items, page_size=(PAGE_W, PAGE_H)) -> str:
    ...
    page.Resources = Dictionary(XObject=Dictionary(**{
        k.lstrip("/"): v for k, v in xobjects.items()}))
...
def gradient_pdf(tmp_path, gradient_src):
    ...
    return build_pdf(tmp_path / "gradient.pdf",
...
def highdpi_pdf(tmp_path, gradient_src):
    ...
    return build_pdf(tmp_path / "highdpi.pdf",
...
def smask_pdf(tmp_path, gradient_src):
    ...
    return build_pdf(tmp_path / "smask.pdf",
                     [(gradient_src, (PAGE_W, PAGE_H, 0, 0), None, mask)])
...
def opaque_smask_pdf(tmp_path, gradient_src):
    ...
    return build_pdf(tmp_path / "opaque.pdf",
                     [(gradient_src, (PAGE_W, PAGE_H, 0, 0), None, mask)])
...
def mixed_pdf(tmp_path, gradient_src):
    ...
    return build_pdf(tmp_path / "mixed.pdf", [
        (gradient_src, (PAGE_W, PAGE_H, 0, 0), None, None),
