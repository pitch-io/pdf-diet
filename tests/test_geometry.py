...
class TestMatrices:
    ...
    def test_translation_then_scale_order_matters(self) -> None:
    ...
    def test_axis_alignment_detection(self) -> None:
    ...
    def test_unit_square_bbox(self) -> None:
        assert bbox_of_unit_square((10, 0, 0, 20, 5, 7)) == (5, 7, 15, 27)


class TestBoxes:
    def test_intersect_treats_none_as_unbounded(self) -> None:
        box = (0, 0, 10, 10)
        assert intersect(None, box) == box
        assert intersect(box, None) == box

    def test_disjoint_intersection_is_empty(self) -> None:
        assert intersect((0, 0, 1, 1), (5, 5, 6, 6)) == (0.0, 0.0, 0.0, 0.0)

    def test_union_grows(self) -> None:
...
class TestPlanner:
    def test_below_threshold_is_left_alone(self) -> None:
    ...
    def test_above_threshold_downsamples_to_target(self) -> None:
    ...
    def test_downsampling_is_per_axis(self) -> None:
    ...
    def test_threshold_is_1_4_times_target(self) -> None:
        assert Web().threshold_dpi == pytest.approx(210.0)
        assert MinimalFileSize().threshold_dpi == pytest.approx(182.0)

    def test_resolution_none_disables_downsampling(self) -> None:
        profile = Web()
    ...
    def test_worst_case_dpi_across_placements_wins(self) -> None:
        ...
        assert by_small_only is not None
        assert plan is not None
        assert plan.size == by_small_only.size

    def test_clip_produces_a_crop(self) -> None:
    ...
    def test_rotated_images_are_not_cropped(self) -> None:
        rotated = Placement(
            objgen=(1, 0),
    ...
    def test_degenerate_images_are_skipped(self) -> None:
        assert plan_image([_placement((1, 1), 10.0, 10.0)], Web()) is None
        assert plan_image([], Web()) is None

    def test_crop_disabled_by_profile(self) -> None:
        profile = Web()
...
class TestScanning:
    def test_finds_the_placement(self, gradient_pdf: Path) -> None:
    ...
    def test_records_the_clip(self, clipped_pdf: Path) -> None:
    ...
    def test_two_images_two_entries(self, mixed_pdf: Path) -> None:
